...
thread_sleep(int64_t wake_up_time) {
    ...
    list_insert_ordered(&sleep_list, &current->sleep_elem, (list_less_func *) sleep_less_func, NULL);
...
init_thread(struct thread *t, const char *name, int priority) {
    ...
    if (t != initial_thread) {
        ...
        t->pid_entry_by_parent = malloc(sizeof(struct pid_entry));
    }
...
void thread_donate_priority(void) {
    ...
    while (cur->blocking_lock != NULL) {
        ...
        if (holding != NULL && holding->priority < cur->priority)
...
int insert_into_fdt(struct file *f){
    ...
    if (fdt_new_entry != NULL) {
        ...
        current->current_fd++;
        fdt_new_entry->file_ptr = f;
        //pushing to descriptor table
        list_push_back(&current->fdt, &fdt_new_entry->elem);
...
/* puts new child into the list children */
void insert_into_children(pid_t child_pid){
    struct pid_entry *new_child = malloc(sizeof(struct pid_entry));
...
void set_status(int status){
    ...
    if (parent != NULL && parent->status != THREAD_DYING) {
        pid_t child_pid = thread_current()->pid;
        struct list_elem *e;
